...
type CommunityQuery struct {
	...
	withOwner            *UserQuery
	withFromTransactions *TransactionQuery
	withToTransactions   *TransactionQuery
...
func (cq *CommunityQuery) QueryWallets() *WalletQuery {
	query := (&WalletClient{config: cq.config}).Query()
	query.path = func(ctx context.Context) (fromU *sql.Selector, err error) {
		...
		step := sqlgraph.NewStep(
			...
			sqlgraph.Edge(sqlgraph.O2M, false, community.WalletsTable, community.WalletsColumn),
		)
...
func (cq *CommunityQuery) QueryOwner() *UserQuery {
	...
	query.path = func(ctx context.Context) (fromU *sql.Selector, err error) {
		...
		step := sqlgraph.NewStep(
			...
			sqlgraph.Edge(sqlgraph.M2O, true, community.OwnerTable, community.OwnerColumn),
		)
		fromU = sqlgraph.SetNeighbors(cq.driver.Dialect(), step)
		return fromU, nil
	}
...
// QueryFromTransactions chains the current query on the "from_transactions" edge.
func (cq *CommunityQuery) QueryFromTransactions() *TransactionQuery {
	...
	query.path = func(ctx context.Context) (fromU *sql.Selector, err error) {
		...
		step := sqlgraph.NewStep(
			...
			sqlgraph.Edge(sqlgraph.O2M, false, community.FromTransactionsTable, community.FromTransactionsColumn),
		)
		fromU = sqlgraph.SetNeighbors(cq.driver.Dialect(), step)
		return fromU, nil
	}
...
// QueryToTransactions chains the current query on the "to_transactions" edge.
func (cq *CommunityQuery) QueryToTransactions() *TransactionQuery {
	query := (&TransactionClient{config: cq.config}).Query()
	query.path = func(ctx context.Context) (fromU *sql.Selector, err error) {
		if err := cq.prepareQuery(ctx); err != nil {
			return nil, err
		...
		step := sqlgraph.NewStep(
			sqlgraph.From(community.Table, community.FieldID, selector),
			sqlgraph.To(transaction.Table, transaction.FieldID),
			sqlgraph.Edge(sqlgraph.O2M, false, community.ToTransactionsTable, community.ToTransactionsColumn),
		)
		fromU = sqlgraph.SetNeighbors(cq.driver.Dialect(), step)
		return fromU, nil
	}
...
func (cq *CommunityQuery) Clone() *CommunityQuery {
	...
	return &CommunityQuery{
		...
		withFromTransactions: cq.withFromTransactions.Clone(),
		withToTransactions:   cq.withToTransactions.Clone(),
...
func (cq *CommunityQuery) WithWallets(opts ...func(*WalletQuery)) *CommunityQuery {
	query := (&WalletClient{config: cq.config}).Query()
...
func (cq *CommunityQuery) WithOwner(opts ...func(*UserQuery)) *CommunityQuery {
	query := (&UserClient{config: cq.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
...
// WithFromTransactions tells the query-builder to eager-load the nodes that are connected to
// the "from_transactions" edge. The optional arguments are used to configure the query builder of the edge.
func (cq *CommunityQuery) WithFromTransactions(opts ...func(*TransactionQuery)) *CommunityQuery {
	query := (&TransactionClient{config: cq.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
	cq.withFromTransactions = query
	return cq
}

// WithToTransactions tells the query-builder to eager-load the nodes that are connected to
// the "to_transactions" edge. The optional arguments are used to configure the query builder of the edge.
func (cq *CommunityQuery) WithToTransactions(opts ...func(*TransactionQuery)) *CommunityQuery {
	query := (&TransactionClient{config: cq.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
	cq.withToTransactions = query
	return cq
}
...
func (cq *CommunityQuery) sqlAll(ctx context.Context, hooks ...queryHook) ([]*Community, error) {
	var (
		...
		loadedTypes = [4]bool{
			cq.withWallets != nil,
			cq.withOwner != nil,
			cq.withFromTransactions != nil,
			cq.withToTransactions != nil,
		}
	)
	...
	if query := cq.withWallets; query != nil {
		if err := cq.loadWallets(ctx, query, nodes,
			func(n *Community) { n.Edges.Wallets = []*Wallet{} },
			func(n *Community, e *Wallet) { n.Edges.Wallets = append(n.Edges.Wallets, e) }); err != nil {
			return nil, err
		}
	}
	if query := cq.withOwner; query != nil {
		if err := cq.loadOwner(ctx, query, nodes, nil,
			func(n *Community, e *User) { n.Edges.Owner = e }); err != nil {
			return nil, err
		}
	}
	if query := cq.withFromTransactions; query != nil {
		if err := cq.loadFromTransactions(ctx, query, nodes,
			func(n *Community) { n.Edges.FromTransactions = []*Transaction{} },
			func(n *Community, e *Transaction) { n.Edges.FromTransactions = append(n.Edges.FromTransactions, e) }); err != nil {
			return nil, err
		}
	}
	if query := cq.withToTransactions; query != nil {
		if err := cq.loadToTransactions(ctx, query, nodes,
			func(n *Community) { n.Edges.ToTransactions = []*Transaction{} },
			func(n *Community, e *Transaction) { n.Edges.ToTransactions = append(n.Edges.ToTransactions, e) }); err != nil {
			return nil, err
		}
	}
	return nodes, nil
...
func (cq *CommunityQuery) loadWallets(ctx context.Context, query *WalletQuery, nodes []*Community, init func(*Community), assign func(*Community, *Wallet)) error {
	fks := make([]driver.Value, 0, len(nodes))
	nodeids := make(map[int]*Community)
	for i := range nodes {
	...
	if len(query.ctx.Fields) > 0 {
		query.ctx.AppendFieldOnce(wallet.FieldCommunityID)
	}
	query.Where(predicate.Wallet(func(s *sql.Selector) {
		s.Where(sql.InValues(s.C(community.WalletsColumn), fks...))
	}))
...
func (cq *CommunityQuery) loadOwner(ctx context.Context, query *UserQuery, nodes []*Community, init func(*Community), assign func(*Community, *User)) error {
	ids := make([]uuid.UUID, 0, len(nodes))
	nodeids := make(map[uuid.UUID][]*Community)
	for i := range nodes {
...
func (cq *CommunityQuery) loadFromTransactions(ctx context.Context, query *TransactionQuery, nodes []*Community, init func(*Community), assign func(*Community, *Transaction)) error {
	fks := make([]driver.Value, 0, len(nodes))
	nodeids := make(map[int]*Community)
	for i := range nodes {
		fks = append(fks, nodes[i].ID)
	...
	if len(query.ctx.Fields) > 0 {
		query.ctx.AppendFieldOnce(transaction.FieldFromCommunityID)
	}
	query.Where(predicate.Transaction(func(s *sql.Selector) {
		s.Where(sql.InValues(s.C(community.FromTransactionsColumn), fks...))
	}))
	neighbors, err := query.All(ctx)
	if err != nil {
		return err
	...
	for _, n := range neighbors {
		fk := n.FromCommunityID
		node, ok := nodeids[fk]
		if !ok {
			return fmt.Errorf(`unexpected referenced foreign-key "from_community_id" returned %v for node %v`, fk, n.ID)
		}
		assign(node, n)
	}
	return nil
}
func (cq *CommunityQuery) loadToTransactions(ctx context.Context, query *TransactionQuery, nodes []*Community, init func(*Community), assign func(*Community, *Transaction)) error {
	fks := make([]driver.Value, 0, len(nodes))
	nodeids := make(map[int]*Community)
	for i := range nodes {
		fks = append(fks, nodes[i].ID)
	...
	if len(query.ctx.Fields) > 0 {
		query.ctx.AppendFieldOnce(transaction.FieldToCommunityID)
	}
	query.Where(predicate.Transaction(func(s *sql.Selector) {
		s.Where(sql.InValues(s.C(community.ToTransactionsColumn), fks...))
	}))
	neighbors, err := query.All(ctx)
	if err != nil {
		return err
	...
	for _, n := range neighbors {
		...
		node, ok := nodeids[fk]
		if !ok {
			return fmt.Errorf(`unexpected referenced foreign-key "to_community_id" returned %v for node %v`, fk, n.ID)
		}
		assign(node, n)
	}
	return nil
...
func (cq *CommunityQuery) querySpec() *sqlgraph.QuerySpec {
	...
	if fields := cq.ctx.Fields; len(fields) > 0 {
		...
		if cq.withOwner != nil {
			_spec.Node.AddColumnOnce(community.FieldOwnerUserID)
		}
	}
