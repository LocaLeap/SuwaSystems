...
type UserQuery struct {
	...
	withOwnerCommunities *CommunityQuery
...
// QueryOwnerCommunities chains the current query on the "owner_communities" edge.
func (uq *UserQuery) QueryOwnerCommunities() *CommunityQuery {
...
func (uq *UserQuery) Clone() *UserQuery {
	...
	return &UserQuery{
		...
		withOwnerCommunities: uq.withOwnerCommunities.Clone(),
...
// WithOwnerCommunities tells the query-builder to eager-load the nodes that are connected to
// the "owner_communities" edge. The optional arguments are used to configure the query builder of the edge.
func (uq *UserQuery) WithOwnerCommunities(opts ...func(*CommunityQuery)) *UserQuery {
	query := (&CommunityClient{config: uq.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
	uq.withOwnerCommunities = query
	return uq
}
...
func (uq *UserQuery) sqlAll(ctx context.Context, hooks ...queryHook) ([]*User, error) {
	...
	if query := uq.withOwnerCommunities; query != nil {
		if err := uq.loadOwnerCommunities(ctx, query, nodes,
			func(n *User) { n.Edges.OwnerCommunities = []*Community{} },
			func(n *User, e *Community) { n.Edges.OwnerCommunities = append(n.Edges.OwnerCommunities, e) }); err != nil {
			return nil, err
		}
	}
	return nodes, nil
...
func (uq *UserQuery) loadOwnerCommunities(ctx context.Context, query *CommunityQuery, nodes []*User, init func(*User), assign func(*User, *Community)) error {
	fks := make([]driver.Value, 0, len(nodes))
	nodeids := make(map[uuid.UUID]*User)
	for i := range nodes {
		fks = append(fks, nodes[i].ID)
	...
	if len(query.ctx.Fields) > 0 {
		query.ctx.AppendFieldOnce(community.FieldOwnerUserID)
	}
	query.Where(predicate.Community(func(s *sql.Selector) {
		s.Where(sql.InValues(s.C(user.OwnerCommunitiesColumn), fks...))
	}))
	neighbors, err := query.All(ctx)
	if err != nil {
		return err
