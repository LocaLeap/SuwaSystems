...
func (uc *UserCreate) AddOwnerCommunityIDs(ids ...int) *UserCreate {
	uc.mutation.AddOwnerCommunityIDs(ids...)
	return uc
}

// AddOwnerCommunities adds the "owner_communities" edges to the Community entity.
func (uc *UserCreate) AddOwnerCommunities(c ...*Community) *UserCreate {
	ids := make([]int, len(c))
	for i := range c {
		ids[i] = c[i].ID
	}
	return uc.AddOwnerCommunityIDs(ids...)
}
...
func (uc *UserCreate) createSpec() (*User, *sqlgraph.CreateSpec) {
	...
	if nodes := uc.mutation.OwnerCommunitiesIDs(); len(nodes) > 0 {
		edge := &sqlgraph.EdgeSpec{
			Rel:     sqlgraph.O2M,
			Inverse: false,
