...
// HasOwnerCommunities applies the HasEdge predicate on the "owner_communities" edge.
func HasOwnerCommunities() predicate.User {
	return predicate.User(func(s *sql.Selector) {
		step := sqlgraph.NewStep(
			sqlgraph.From(Table, FieldID),
...
// HasOwnerCommunitiesWith applies the HasEdge predicate on the "owner_communities" edge with a given conditions (other predicates).
func HasOwnerCommunitiesWith(preds ...predicate.Community) predicate.User {
	return predicate.User(func(s *sql.Selector) {
		step := newOwnerCommunitiesStep()
		sqlgraph.HasNeighborsWith(s, step, func(s *sql.Selector) {
			for _, p := range preds {
				p(s)
			}
