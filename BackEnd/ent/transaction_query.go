...
type TransactionQuery struct {
	...
	withFromCommunity *CommunityQuery
	withToCommunity   *CommunityQuery
	withFromWallet    *WalletQuery
	withToWallet      *WalletQuery
...
// QueryFromCommunity chains the current query on the "from_community" edge.
func (tq *TransactionQuery) QueryFromCommunity() *CommunityQuery {
	query := (&CommunityClient{config: tq.config}).Query()
	query.path = func(ctx context.Context) (fromU *sql.Selector, err error) {
		...
		step := sqlgraph.NewStep(
			...
			sqlgraph.Edge(sqlgraph.M2O, true, transaction.FromCommunityTable, transaction.FromCommunityColumn),
		)
...
// QueryToCommunity chains the current query on the "to_community" edge.
func (tq *TransactionQuery) QueryToCommunity() *CommunityQuery {
	query := (&CommunityClient{config: tq.config}).Query()
	query.path = func(ctx context.Context) (fromU *sql.Selector, err error) {
		if err := tq.prepareQuery(ctx); err != nil {
			return nil, err
		...
		step := sqlgraph.NewStep(
			sqlgraph.From(transaction.Table, transaction.FieldID, selector),
			sqlgraph.To(community.Table, community.FieldID),
			sqlgraph.Edge(sqlgraph.M2O, true, transaction.ToCommunityTable, transaction.ToCommunityColumn),
		)
		fromU = sqlgraph.SetNeighbors(tq.driver.Dialect(), step)
		return fromU, nil
	}
...
// QueryFromWallet chains the current query on the "from_wallet" edge.
func (tq *TransactionQuery) QueryFromWallet() *WalletQuery {
	query := (&WalletClient{config: tq.config}).Query()
	query.path = func(ctx context.Context) (fromU *sql.Selector, err error) {
		if err := tq.prepareQuery(ctx); err != nil {
			return nil, err
		}
		...
		step := sqlgraph.NewStep(
			...
			sqlgraph.Edge(sqlgraph.M2O, true, transaction.FromWalletTable, transaction.FromWalletColumn),
		)
		fromU = sqlgraph.SetNeighbors(tq.driver.Dialect(), step)
		return fromU, nil
	}
...
// QueryToWallet chains the current query on the "to_wallet" edge.
func (tq *TransactionQuery) QueryToWallet() *WalletQuery {
	query := (&WalletClient{config: tq.config}).Query()
	query.path = func(ctx context.Context) (fromU *sql.Selector, err error) {
		if err := tq.prepareQuery(ctx); err != nil {
			return nil, err
		...
		step := sqlgraph.NewStep(
			sqlgraph.From(transaction.Table, transaction.FieldID, selector),
			sqlgraph.To(wallet.Table, wallet.FieldID),
			sqlgraph.Edge(sqlgraph.M2O, true, transaction.ToWalletTable, transaction.ToWalletColumn),
		)
		fromU = sqlgraph.SetNeighbors(tq.driver.Dialect(), step)
		return fromU, nil
	}
...
func (tq *TransactionQuery) Clone() *TransactionQuery {
	...
	return &TransactionQuery{
		...
		withFromCommunity: tq.withFromCommunity.Clone(),
		withToCommunity:   tq.withToCommunity.Clone(),
		withFromWallet:    tq.withFromWallet.Clone(),
		withToWallet:      tq.withToWallet.Clone(),
...
// WithFromCommunity tells the query-builder to eager-load the nodes that are connected to
// the "from_community" edge. The optional arguments are used to configure the query builder of the edge.
func (tq *TransactionQuery) WithFromCommunity(opts ...func(*CommunityQuery)) *TransactionQuery {
	query := (&CommunityClient{config: tq.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
	tq.withFromCommunity = query
	return tq
}

// WithToCommunity tells the query-builder to eager-load the nodes that are connected to
// the "to_community" edge. The optional arguments are used to configure the query builder of the edge.
func (tq *TransactionQuery) WithToCommunity(opts ...func(*CommunityQuery)) *TransactionQuery {
	query := (&CommunityClient{config: tq.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
	tq.withToCommunity = query
	return tq
}

// WithFromWallet tells the query-builder to eager-load the nodes that are connected to
// the "from_wallet" edge. The optional arguments are used to configure the query builder of the edge.
func (tq *TransactionQuery) WithFromWallet(opts ...func(*WalletQuery)) *TransactionQuery {
	query := (&WalletClient{config: tq.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
	tq.withFromWallet = query
	return tq
}

// WithToWallet tells the query-builder to eager-load the nodes that are connected to
// the "to_wallet" edge. The optional arguments are used to configure the query builder of the edge.
func (tq *TransactionQuery) WithToWallet(opts ...func(*WalletQuery)) *TransactionQuery {
	query := (&WalletClient{config: tq.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
	tq.withToWallet = query
	return tq
}
...
func (tq *TransactionQuery) sqlAll(ctx context.Context, hooks ...queryHook) ([]*Transaction, error) {
	var (
		...
		_spec       = tq.querySpec()
		loadedTypes = [4]bool{
			tq.withFromCommunity != nil,
			tq.withToCommunity != nil,
			tq.withFromWallet != nil,
			tq.withToWallet != nil,
		}
	)
	...
	if query := tq.withFromCommunity; query != nil {
		if err := tq.loadFromCommunity(ctx, query, nodes, nil,
			func(n *Transaction, e *Community) { n.Edges.FromCommunity = e }); err != nil {
			return nil, err
		}
	}
	if query := tq.withToCommunity; query != nil {
		if err := tq.loadToCommunity(ctx, query, nodes, nil,
			func(n *Transaction, e *Community) { n.Edges.ToCommunity = e }); err != nil {
			return nil, err
		}
	}
	if query := tq.withFromWallet; query != nil {
		if err := tq.loadFromWallet(ctx, query, nodes, nil,
			func(n *Transaction, e *Wallet) { n.Edges.FromWallet = e }); err != nil {
			return nil, err
		}
	}
	if query := tq.withToWallet; query != nil {
		if err := tq.loadToWallet(ctx, query, nodes, nil,
			func(n *Transaction, e *Wallet) { n.Edges.ToWallet = e }); err != nil {
			return nil, err
		}
	}
	return nodes, nil
...
func (tq *TransactionQuery) loadFromCommunity(ctx context.Context, query *CommunityQuery, nodes []*Transaction, init func(*Transaction), assign func(*Transaction, *Community)) error {
	ids := make([]int, 0, len(nodes))
	nodeids := make(map[int][]*Transaction)
	for i := range nodes {
		fk := nodes[i].FromCommunityID
	...
	for _, n := range neighbors {
		...
		if !ok {
			return fmt.Errorf(`unexpected foreign-key "from_community_id" returned %v`, n.ID)
		}
		for i := range nodes {
			assign(nodes[i], n)
		}
...
func (tq *TransactionQuery) loadToCommunity(ctx context.Context, query *CommunityQuery, nodes []*Transaction, init func(*Transaction), assign func(*Transaction, *Community)) error {
	ids := make([]int, 0, len(nodes))
	nodeids := make(map[int][]*Transaction)
	for i := range nodes {
		fk := nodes[i].ToCommunityID
		if _, ok := nodeids[fk]; !ok {
			ids = append(ids, fk)
		}
		nodeids[fk] = append(nodeids[fk], nodes[i])
	...
	for _, n := range neighbors {
		nodes, ok := nodeids[n.ID]
		if !ok {
			return fmt.Errorf(`unexpected foreign-key "to_community_id" returned %v`, n.ID)
		}
		for i := range nodes {
			assign(nodes[i], n)
		}
...
func (tq *TransactionQuery) loadFromWallet(ctx context.Context, query *WalletQuery, nodes []*Transaction, init func(*Transaction), assign func(*Transaction, *Wallet)) error {
	ids := make([]int, 0, len(nodes))
	nodeids := make(map[int][]*Transaction)
	for i := range nodes {
		fk := nodes[i].FromWalletID
		if _, ok := nodeids[fk]; !ok {
			ids = append(ids, fk)
		}
		nodeids[fk] = append(nodeids[fk], nodes[i])
	...
	for _, n := range neighbors {
		nodes, ok := nodeids[n.ID]
		if !ok {
			return fmt.Errorf(`unexpected foreign-key "from_wallet_id" returned %v`, n.ID)
		}
		for i := range nodes {
			assign(nodes[i], n)
		}
...
func (tq *TransactionQuery) loadToWallet(ctx context.Context, query *WalletQuery, nodes []*Transaction, init func(*Transaction), assign func(*Transaction, *Wallet)) error {
	ids := make([]int, 0, len(nodes))
	nodeids := make(map[int][]*Transaction)
	for i := range nodes {
		fk := nodes[i].ToWalletID
		if _, ok := nodeids[fk]; !ok {
			ids = append(ids, fk)
		}
		nodeids[fk] = append(nodeids[fk], nodes[i])
	...
	for _, n := range neighbors {
		nodes, ok := nodeids[n.ID]
		if !ok {
			return fmt.Errorf(`unexpected foreign-key "to_wallet_id" returned %v`, n.ID)
		}
		for i := range nodes {
			assign(nodes[i], n)
		}
...
func (tq *TransactionQuery) querySpec() *sqlgraph.QuerySpec {
	...
	if fields := tq.ctx.Fields; len(fields) > 0 {
		...
		if tq.withFromCommunity != nil {
			_spec.Node.AddColumnOnce(transaction.FieldFromCommunityID)
		}
		if tq.withToCommunity != nil {
			_spec.Node.AddColumnOnce(transaction.FieldToCommunityID)
		}
		if tq.withFromWallet != nil {
			_spec.Node.AddColumnOnce(transaction.FieldFromWalletID)
		}
		if tq.withToWallet != nil {
			_spec.Node.AddColumnOnce(transaction.FieldToWalletID)
		}
	}
